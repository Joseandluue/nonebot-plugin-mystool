...
__all__ = [
    ...
    "manually_starrail_note_check","manually_weibo_check","manually_rrjf"
]
...
async def key_rrjf(event: Union[GeneralMessageEvent], matcher: Matcher):
    ...
    user_id = event.get_user_id()
    user = PluginDataManager.plugin_data.users.get(user_id)
    await api_rrjf(user=user, user_ids=[user_id], matcher=matcher)


async def api_rrjf(user: UserData, user_ids: Iterable[str],matcher: Matcher = None):
    appkey = user.geetest_params.get("appkey")
